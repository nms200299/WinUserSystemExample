/*
Title : ALL-IN-ONE EAT 후킹 테스트
Summary : 스스로의 EAT를 조작하여 EAT 후킹을 테스트합니다.
*/ 
...
int _tmain() {	
	...
	Msg(); // EAT 후킹 전
	printf("EAT 후킹 후\n");
	DWORD dwOrgEatMessageBoxW = EatHooking(L"USER32.dll", "MessageBoxW", (DWORD_PTR)MessageBoxW_Hook, FALSE);
	DWORD dwOrgEatMessageBoxA = EatHooking(L"USER32.dll", "MessageBoxA", (DWORD_PTR)MessageBoxA_Hook, FALSE);
	Msg(); // EAT 후킹 후
	printf("EAT 후킹 원상 복구\n");
	...
	Msg(); // EAT 원상 복구
	printf("EAT 재후킹\n");
	EatHooking(L"USER32.dll", "MessageBoxW", (DWORD_PTR)MessageBoxW_Hook, FALSE);
	EatHooking(L"USER32.dll", "MessageBoxA", (DWORD_PTR)MessageBoxA_Hook, FALSE);
	Msg(); // EAT 재후킹
	system("pause");
}
